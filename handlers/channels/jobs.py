...
async def daily_fatwa_job(context: ContextTypes.DEFAULT_TYPE, force: bool = False, respect_scheduled: bool = True):
    ...
    if respect_scheduled:
        ...
        if fatwa: scheduled_used, mode_label = True, "جدولة فتوى لمرة واحدة"

    if fatwa is None and not force and not random_enabled and not specific_enabled: return

    if fatwa is None:
        if specific_enabled:
            target_cat_id, _ = await _get_selected_publish_category()
            if not target_cat_id: return
    ...
    try:
        ...
        for admin in admins:
            try: await context.bot.send_message(chat_id=admin['user_id'], text=report, parse_mode='Markdown', reply_markup=_build_delivery_report_keyboard(fatwa['id']))
            except: pass
    finally:
