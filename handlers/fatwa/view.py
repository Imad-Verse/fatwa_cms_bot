...
from .utils import _build_view_back_button, _extract_context_suffix, _resolve_view_context_data
...
async def show_random_fatwa(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    fatwa = await db.get_random_fatwa(public_only=public_only, excluded_fatwa_ids=excluded_ids)
...
async def view_fatwa(
    ...
    try:
        ...
        is_favorite = await bot_db.is_favorite(update.effective_user.id, fatwa_id)
...
async def show_related_fatwas(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    public_only = not await bot_db.is_admin(update.effective_user.id)
    related = await db.get_related_fatwas(fatwa_id, limit=5, public_only=public_only)
...
async def send_fatwa_message(update: Update, context: ContextTypes.DEFAULT_TYPE, fatwa_id: int):
    ...
    try:
        ...
        action_buttons = []
        is_fav = await bot_db.is_favorite(update.effective_user.id, fatwa_id)
