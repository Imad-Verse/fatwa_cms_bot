...
from .utils import _register_delivery_message, _build_delivery_report_keyboard
...
async def process_broadcast_task(
    ...
    async def send_with_retry(chat_id, category_key, is_chat_dict=False):
        ...
        for attempt in range(3):
            try:
                ...
                err = str(e)
                if is_chat_dict:
                    await bot_db.update_channel_status(chat_id, 'inactive')
                elif chat_id != admin_id:
                    await bot_db.set_user_blocked(chat_id, True)
                break
    ...
    if approx_views > 0:
        await db.increment_views_by(fatwa_id, approx_views)
...
async def broadcast_fatwa(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if maintenance_enabled:
        ...
        users = list(dict.fromkeys(await bot_db.get_active_user_ids()))
