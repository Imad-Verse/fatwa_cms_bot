...
async def handle_edit_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("edit_tax_cat_") or data.startswith("add_another_cat_"):
        ...
        await show_edit_categories_step(update, context); return STATE_EDIT_CATEGORY
    ...
    if data.startswith("edit_tax_top_"):
        ...
        if not cls_list: await query.answer("⚠️ لا يوجد تصنيف لهذا النوع. اختر تصنيفاً أولاً.", show_alert=True); return STATE_EDIT_MENU
        cat_id = cls_list[0]['category_id']; context.user_data['edit_topic_cat_id'] = cat_id
        await show_edit_topics_step(update, context, cat_id=cat_id); return STATE_EDIT_TOPIC
...
async def handle_edit_category_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "edit_cat_search_cancel": context.user_data.pop(f'edit_cat_search_{slot}', None); await show_edit_categories_step(update, context, page=0); return STATE_EDIT_CATEGORY
    if data == "add_new_edit_cat": await query.edit_message_text("🏷️ أرسل اسم التصنيف الجديد:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 إلغاء", callback_data="cancel_new_cat")]])); return STATE_EDIT_NEW_CAT
    if data == "cancel_new_cat": await show_edit_categories_step(update, context, page=0); return STATE_EDIT_CATEGORY
    if data.startswith("edit_cat_page_"): await show_edit_categories_step(update, context, page=int(data.split('_')[-1])); return STATE_EDIT_CATEGORY
...
async def handle_edit_topic_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "edit_topic_search_cancel": context.user_data.pop(f'edit_topic_search_{cat_id}', None); await show_edit_topics_step(update, context, cat_id=cat_id, page=0); return STATE_EDIT_TOPIC
    if data == "add_new_edit_topic": await query.edit_message_text("📑 أرسل اسم الموضوع الجديد:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 إلغاء", callback_data="cancel_new_topic")]])); return STATE_EDIT_NEW_TOP
    if data == "cancel_new_topic": await show_edit_topics_step(update, context, cat_id=cat_id, page=0); return STATE_EDIT_TOPIC
    if data.startswith("edit_top_page_"): await show_edit_topics_step(update, context, cat_id=cat_id, page=int(data.split('_')[-1])); return STATE_EDIT_TOPIC
...
async def handle_edit_cat_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await show_edit_categories_step(update, context, search_query=q); return STATE_EDIT_CATEGORY
...
async def handle_edit_topic_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await show_edit_topics_step(update, context, cat_id=cid, search_query=q); return STATE_EDIT_TOPIC
...
async def show_edit_categories_step(update, context, page=0, search_query=None):
    ...
    if offset + ITEMS_PER_PAGE < total: nav.append(InlineKeyboardButton("➡️ التالي", callback_data=f"edit_cat_page_{page+1}"))
    if nav: keyboard.append(nav)
...
async def handle_receive_new_topic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not cid: return STATE_EDIT_TOPIC
